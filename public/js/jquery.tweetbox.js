(function($){
	...
	var methods={
	    ...
	    this.$editor=$('<div contenteditable="true" id="editor" class="tweet-box-editable">').css(this.settings.css).css(cover).css({bottom:'40px',left:'0px','z-index':'99','overflow-x':'hidden','overflow-y':'auto','white-space':'pre',padding:'10px'});
	    $(this).css({overflow:'hidden',padding:'100px'}).html(this.$editor);
	    ...
	    setTimeout(function(){
		  ...
		  this.$button=$('<input type="button" id="tweet-box-button" class="tweet-box-button" value="Tweet">').css({position:'absolute',top:'4px',bottom:'5px',right:'1%',width:'20%',overflow:'hidden', height:'75%', padding:'2px'}).html(this.settings.button[this.settings.action]);
